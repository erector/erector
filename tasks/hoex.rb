# This is a patched version of hoe.rb 1.5.0
# by Alex
# I've taken out some of the tasks and made others more configurable
...
class Hoe
  ...
  ##
  # Optional: The directory in which to generate rdocs (default: 'doc')
  attr_accessor :rdoc_dir
  
  ##
  # Optional: The directory containing the project web site (default: 'web')
  attr_accessor :web_dir
  
  ##
  # Optional: An array of globs of files to include in the gem. If not set, we use Manifest.txt.
  attr_accessor :files
  ...
  def define_tasks # :nodoc:
    ...
    task :release => [:clean, :package] do |t|
      v = ENV["VERSION"] or abort "Please specify VERSION=x.y.z on the command line (to assure we're releasing the right version)"
...
class Hoe
  def self.remove_tasks(*args)
    args.each do |task_name|
      Rake.application.task_hash.delete(task_name)
    end
  end
end
